...
public class AppShortcutsHelper {
    ...
    /** Rebuilds Android dynamic shortcuts from the preference set restored by a backup. */
    public static void syncConfiguredAppShortcuts(Context context, List<RemoteItem> remotes) {
        ...
        String preferenceKey = context.getString(R.string.shared_preferences_app_shortcuts);
        ...
        for (RemoteItem remoteItem : remotes) {
            ...
            shortcuts.add(new ShortcutInfo.Builder(context, id)
                    ...
                    .setIcon(Icon.createWithResource(
                            context,
                            getRemoteIcon(remoteItem.getType(), remoteItem.isCrypt())))
                    .setIntent(intent)
                    .build());
            restoredIds.add(id);
            if (shortcuts.size() >= 4) {
        ...
        preferences.edit().putStringSet(preferenceKey, restoredIds).apply();
    }
