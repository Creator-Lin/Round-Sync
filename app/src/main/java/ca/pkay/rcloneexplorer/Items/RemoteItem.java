...
public class RemoteItem implements Comparable<RemoteItem>, Parcelable {
    ...
    /** Cookie-driven QuarkDav kernel managed independently from rclone. */
    public static final int QUARKDAV_COOKIE = -20;
    ...
    public boolean isQuarkDav() {
        return type == QUARKDAV_COOKIE || "quarkdav-cookie".equals(typeReadable);
    }

    public String getQuarkDavId() {
        final String prefix = "quarkdav:";
        return isQuarkDav() && name != null && name.startsWith(prefix) ? name.substring(prefix.length()) : null;
    }
    ...
    private int getTypeFromString(String type) {
        switch (type) {
            case SafConstants.SAF_REMOTE_NAME:
                return SAFW;
            case "quarkdav-cookie":
                return QUARKDAV_COOKIE;
            case "alias":
                return ALIAS;
    ...
    public int getRemoteIcon(int type) {
        if (isCrypt()) {
            ...
            switch (type) {
                ...
                case RemoteItem.QUARKDAV_COOKIE:
                    return R.drawable.ic_quarkdav_core;
