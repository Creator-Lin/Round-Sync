...
public class StreamingService extends IntentService {
    ...
    public static final String READ_ONLY = "ca.pkay.rcexplorer.read_only";
    ...
    public static final String VIDEO_STREAM_READ_CONFIG = "ca.pkay.rcexplorer.video_stream_read_config";
    public static final String SERVE_BASE_URL = "ca.pkay.rcexplorer.streaming_service.base_url";
    ...
    public StreamingService() {
        ...
        // Re-run the exact serving intent if Android reclaims this foreground process.
    ...
    protected void onHandleIntent(@Nullable Intent intent) {
        ...
        final boolean readOnly = intent.hasExtra(READ_ONLY)
                ? intent.getBooleanExtra(READ_ONLY, false)
                : PreferenceManager.getDefaultSharedPreferences(this)
                        .getBoolean(getString(R.string.pref_choice_serve_dialog_read_only), false);
        ...
        final boolean useVideoStreamReadConfig = intent.getBooleanExtra(VIDEO_STREAM_READ_CONFIG, false);
        final String serveBaseUrl = intent.getStringExtra(SERVE_BASE_URL);
        ...
        if (showNotificationText) {
            Uri uri = buildLocalHttpUri(port, serveBaseUrl);
        ...
        try {
            switch (protocol) {
                case SERVE_FTP:
                    runningProcess = rclone.serve(Rclone.SERVE_PROTOCOL_FTP, port, allowRemoteAccess, authenticationUsername, authenticationPassword, remote, servePath, readOnly);
                    break;
                case SERVE_WEBDAV:
                    runningProcess = rclone.serve(Rclone.SERVE_PROTOCOL_WEBDAV, port, allowRemoteAccess, authenticationUsername, authenticationPassword, remote, servePath, readOnly);
                    break;
                case SERVE_DLNA:
                    runningProcess = rclone.serve(Rclone.SERVE_PROTOCOL_DLNA, port, allowRemoteAccess, authenticationUsername, authenticationPassword, remote, servePath, readOnly);
                    break;
                case SERVE_HTTP:
                default:
                    runningProcess = rclone.serve(Rclone.SERVE_PROTOCOL_HTTP, port, allowRemoteAccess,
                            authenticationUsername, authenticationPassword, remote, servePath,
                            serveBaseUrl, readOnly, useVideoStreamReadConfig);
                    break;
            }
            ...
            if (runningProcess != null) {
                try {
                    ...
                    if (exitCode != 0) rclone.logErrorOutput(runningProcess);
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                    FLog.e(TAG, "onHandleIntent: interrupted while waiting for process", e);
    ...
    private Uri buildLocalHttpUri(int port, @Nullable String baseUrl) {
        Uri.Builder builder = Uri.parse("http://127.0.0.1:" + port).buildUpon();
        if (baseUrl != null) {
            ...
            if (!normalized.isEmpty()) {
                builder.appendEncodedPath(normalized);
            }
        }
    ...
    private void acquireResourceLocks() {
        ...
        wakeLock = power.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, getPackageName() + ":rclone-streaming");
        wakeLock.setReferenceCounted(false);
        wakeLock.acquire();

        WifiManager wifi = (WifiManager) getApplicationContext().getSystemService(Context.WIFI_SERVICE);
        wifiLock = wifi.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, getPackageName() + ":rclone-streaming-wifi");
    ...
    private void releaseResourceLocks() {
        try {
            ...
            FLog.e(TAG, "releaseResourceLocks: wake lock", e);
        }
        try {
            if (wifiLock != null && wifiLock.isHeld()) wifiLock.release();
        } catch (RuntimeException e) {
            FLog.e(TAG, "releaseResourceLocks: wifi lock", e);
        }
        wakeLock = null;
        wifiLock = null;
    }
