...
class DatabaseHandler(context: Context?) :
    ...
    /** Restores a task with its original ID so trigger targets remain valid. */
    fun createTaskWithId(taskToStore: Task): Task {
        val db = writableDatabase
        val rowId = db.insertOrThrow(Task.TABLE_NAME, null, getTaskContentValuesWithID(taskToStore))
        db.close()
        check(rowId == taskToStore.id) { "Unable to restore task ID ${taskToStore.id}" }
        return taskToStore
    }
    ...
    /** Restores a trigger with its original ID from a configuration backup. */
    fun createTriggerWithId(triggerToStore: Trigger): Trigger {
        val db = writableDatabase
        val rowId = db.insertOrThrow(Trigger.TABLE_NAME, null, getTriggerContentValuesWithID(triggerToStore))
        db.close()
        check(rowId == triggerToStore.id) { "Unable to restore trigger ID ${triggerToStore.id}" }
        return triggerToStore
    }
    ...
    /** Atomically replaces the task/trigger database with a validated backup. */
    fun replaceAll(tasks: List<Task>, triggers: List<Trigger>) {
        val db = writableDatabase
        db.beginTransaction()
        try {
            ...
            tasks.forEach { task ->
                val rowId = db.insertOrThrow(Task.TABLE_NAME, null, getTaskContentValuesWithID(task))
                check(rowId == task.id) { "Unable to restore task ID ${task.id}" }
            }
            triggers.forEach { trigger ->
                val rowId = db.insertOrThrow(Trigger.TABLE_NAME, null, getTriggerContentValuesWithID(trigger))
                check(rowId == trigger.id) { "Unable to restore trigger ID ${trigger.id}" }
            }
