...
import ca.pkay.rcloneexplorer.Services.ThumbnailPreGenerationService;
...
import ca.pkay.rcloneexplorer.widget.AutoFitGridLayoutManager;
...
public class FileExplorerFragment extends Fragment implements   FileExplorerRecyclerViewAdapter.OnClickListener,
    ...
    private static final String LEGACY_SHARED_PREFS_VIEW_MODE = "ca.pkay.rcexplorer.file_view_mode";
    private static final String LEGACY_SHARED_PREFS_THUMBNAIL_SIZE = "ca.pkay.rcexplorer.thumbnail_display_size";
    private static final String SHARED_PREFS_PATH_VIEW_MODE_PREFIX =
            "ca.pkay.rcexplorer.file_view_mode.path.";
    private static final int STREAM_PATH_TOKEN_BYTES = 32;
    ...
    private int thumbnailDisplaySize;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        sharedPreferences.edit()
                .remove(LEGACY_SHARED_PREFS_VIEW_MODE)
                .remove(LEGACY_SHARED_PREFS_THUMBNAIL_SIZE)
                .apply();
        loadFileDisplayModeForPath(path, false);
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        // Entry animations are expensive for dynamic-ratio Grid rows and made
        // Log/File transitions visibly stutter when a view had to be rebound.
        ...
        recyclerViewAdapter.setDisplayMode(fileViewMode, thumbnailDisplaySize);
        recyclerViewAdapter.setWrapFileNames(wrapFilenames);
        recyclerView.setAdapter(recyclerViewAdapter);
        applyFileDisplayMode(false);
        ...
        bindActivityChrome();
        rebuildBreadcrumbsForCurrentPath();
    ...
    /**
     * Restores the controls hosted by MainActivity after the LogFragment overlay
     * is removed. The file list view itself remains alive, so this method only
     * changes lightweight Activity chrome and never reloads directory content.
     */
    public void restoreActivityChrome() {
        if (context == null || getActivity() == null || !isAdded() || isHidden()) {
            return;
        }
        bindActivityChrome();
        setTitle();
        ...
        if (searchBar != null && searchButton != null) {
            searchBar.setVisibility(isSearchMode ? View.VISIBLE : View.INVISIBLE);
            searchButton.setVisibility(isSearchMode ? View.INVISIBLE : View.VISIBLE);
        }
        if (isInMoveMode
                || (recyclerViewAdapter != null && recyclerViewAdapter.isInSelectMode())) {
    ...
    private void bindActivityChrome() {
        FragmentActivity activity = getActivity();
        if (activity == null) {
            return;
        }
        breadcrumbView = activity.findViewById(R.id.breadcrumb_view);
        searchBar = activity.findViewById(R.id.search_bar);
        searchButton = activity.findViewById(R.id.searchButton);
        searchClear = activity.findViewById(R.id.search_clear);
            ...
            searchClear.setOnClickListener(v -> {
                ...
                if (searchField.getText().toString().isEmpty()) {
                    ...
                    if (keyboard != null && focusedView != null) {
                        keyboard.hideSoftInputFromWindow(focusedView.getWindowToken(),
    ...
    private void rebuildBreadcrumbsForCurrentPath() {
        if (breadcrumbView == null || remote == null || directoryObject == null) {
            return;
        }
        String rootPath = "//" + remoteName;
        String currentPath = directoryObject.getCurrentPath();
        breadcrumbView.clearCrumbs();
        breadcrumbView.addCrumb(remote.getDisplayName(), rootPath);
        if (currentPath == null || currentPath.isEmpty() || currentPath.equals(rootPath)) {
            return;
        }

        // Directory paths have two valid forms in this app:
        //   //remote/folder/subfolder  - normal remote-home browsing
        //   /folder/subfolder         - browsing from the remote filesystem root
        // The old restore code only expanded the first form and rendered the
        // second form as one crumb containing slash-separated text.
        PathParts pathParts = splitCurrentPath(rootPath, currentPath);
        String accumulatedPath = pathParts.pathPrefix;
        for (String segment : pathParts.segments) {
            accumulatedPath = appendPathSegment(accumulatedPath, segment);
            breadcrumbView.addCrumb(segment, accumulatedPath);
        }
    }

    private PathParts splitCurrentPath(String rootPath, String currentPath) {
        String pathPrefix;
        String relativePath;
        ...
        return new PathParts(pathPrefix, segments.toArray(new String[0]));
    }

    private String appendPathSegment(String prefix, String segment) {
        if (prefix.isEmpty()) {
            return segment;
        }
        if ("/".equals(prefix)) {
    ...
    private static final class PathParts {
        ...
        private PathParts(String pathPrefix, String[] segments) {
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        if (recyclerViewAdapter != null && recyclerViewAdapter.isInSelectMode()) {
    ...
    private void buildStackFromPath(String remote, String path) {
        String rootPath = "//" + remote;
        pathStack.clear();
        if (path == null || path.isEmpty() || rootPath.equals(path)) {
            return;
        }

        pathStack.push(rootPath);
        PathParts pathParts = splitCurrentPath(rootPath, path);
        String accumulatedPath = pathParts.pathPrefix;
        for (int i = 0; i < pathParts.segments.length - 1; i++) {
            accumulatedPath = appendPathSegment(accumulatedPath, pathParts.segments[i]);
            pathStack.push(accumulatedPath);
        }
    }
    ...
    private void registerReceivers() {
        ...
        intentFilter.addAction(ThumbnailPreGenerationService.ACTION_COMPLETED);
    ...
    private BroadcastReceiver backgroundTaskBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (ThumbnailPreGenerationService.ACTION_COMPLETED.equals(intent.getAction())) {
                String completedRemote = intent.getStringExtra(
                        ThumbnailPreGenerationService.RESULT_REMOTE_NAME);
                String completedType = intent.getStringExtra(
                        ThumbnailPreGenerationService.RESULT_CACHE_TYPE);
                if (remoteName.equals(completedRemote) && recyclerViewAdapter != null) {
                    try {
                        recyclerViewAdapter.notifyThumbnailCacheUpdated(
                                ThumbnailRepository.CacheType.valueOf(completedType));
                    } catch (IllegalArgumentException | NullPointerException ignored) {
                        // Ignore malformed internal completion broadcasts.
                    }
                }
                return;
            }
    ...
    private void setOptionsMenuVisibility(boolean setVisible) {
        if (menuSelectAll == null || menuGoTo == null || menuLink == null
                || menuHttpServe == null || menuEmptyTrash == null) {
            return;
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            ...
            case R.id.action_view_list:
                setFileDisplayMode(FileExplorerRecyclerViewAdapter.VIEW_MODE_LIST,
                        thumbnailDisplaySize, true);
                return true;
            case R.id.action_view_icons_small:
                setFileDisplayMode(FileExplorerRecyclerViewAdapter.VIEW_MODE_GRID,
                        FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_SMALL, true);
                return true;
            case R.id.action_view_icons_medium:
                setFileDisplayMode(FileExplorerRecyclerViewAdapter.VIEW_MODE_GRID,
                        FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_MEDIUM, true);
                return true;
            case R.id.action_view_icons_large:
                setFileDisplayMode(FileExplorerRecyclerViewAdapter.VIEW_MODE_GRID,
                        FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_LARGE, true);
                return true;
            case R.id.action_view_icons_extra_large:
                setFileDisplayMode(FileExplorerRecyclerViewAdapter.VIEW_MODE_GRID,
                        FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_EXTRA_LARGE, true);
                return true;
    ...
    private void setFileDisplayMode(int viewMode, int thumbnailSize, boolean persist) {
        fileViewMode = viewMode;
        thumbnailDisplaySize = thumbnailSize;
        applyFileDisplayMode(true);
        if (persist) {
            persistFileDisplayModeForCurrentPath();
        }
        if (getActivity() != null) {
            getActivity().invalidateOptionsMenu();
        }
    ...
    private void loadFileDisplayModeForPath(String path, boolean apply) {
        fileViewMode = FileExplorerRecyclerViewAdapter.VIEW_MODE_LIST;
        thumbnailDisplaySize = FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_MEDIUM;

        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
        String preferenceKey = getPathViewModePreferenceKey(path);
        if (preferences.contains(preferenceKey)) {
            int savedThumbnailSize = preferences.getInt(preferenceKey,
                    FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_MEDIUM);
            if (savedThumbnailSize >= FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_SMALL
                    && savedThumbnailSize <= FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_EXTRA_LARGE) {
                fileViewMode = FileExplorerRecyclerViewAdapter.VIEW_MODE_GRID;
                thumbnailDisplaySize = savedThumbnailSize;
            } else {
    ...
    private void persistFileDisplayModeForCurrentPath() {
        String path = directoryObject.getCurrentPath();
        ...
        String preferenceKey = getPathViewModePreferenceKey(path);
        if (fileViewMode == FileExplorerRecyclerViewAdapter.VIEW_MODE_LIST) {
            // List is the default. Keep SharedPreferences free of default-only paths.
            editor.remove(preferenceKey);
        } else {
            editor.putInt(preferenceKey, thumbnailDisplaySize);
        }
        editor.apply();
    }

    private String getPathViewModePreferenceKey(String path) {
        if (path == null) {
            path = "";
        }
        String encodedPath = Base64.encodeToString(path.getBytes(StandardCharsets.UTF_8),
                Base64.URL_SAFE | Base64.NO_WRAP | Base64.NO_PADDING);
        return SHARED_PREFS_PATH_VIEW_MODE_PREFIX + encodedPath;
    }

    private void setCurrentDirectoryPath(String path) {
        directoryObject.setPath(path);
        loadFileDisplayModeForPath(path, true);
    }

    private void restoreCurrentDirectoryFromCache(String path) {
        directoryObject.restoreFromCache(path);
        loadFileDisplayModeForPath(path, true);
    }

    private void applyFileDisplayMode(boolean preservePosition) {
        if (recyclerView == null || recyclerViewAdapter == null) {
            return;
        }
        int firstVisible = 0;
        int offset = 0;
        if (preservePosition && recyclerViewLinearLayoutManager != null) {
            firstVisible = Math.max(0, recyclerViewLinearLayoutManager.findFirstVisibleItemPosition());
            View firstView = recyclerViewLinearLayoutManager.findViewByPosition(firstVisible);
            if (firstView != null) {
                offset = firstView.getTop() - recyclerView.getPaddingTop();
            }
        }

        recyclerViewAdapter.setDisplayMode(fileViewMode, thumbnailDisplaySize);
        updateExplorerContentWidthForDisplayMode();
        if (fileViewMode == FileExplorerRecyclerViewAdapter.VIEW_MODE_GRID) {
            int columnWidthPx = Math.round(recyclerViewAdapter.getGridColumnWidthDp()
                    * getResources().getDisplayMetrics().density);
            recyclerViewLinearLayoutManager = new AutoFitGridLayoutManager(context, columnWidthPx);
        } else {
        ...
        if (preservePosition && recyclerViewAdapter.getItemCount() > 0) {
            recyclerViewLinearLayoutManager.scrollToPositionWithOffset(
                    Math.min(firstVisible, recyclerViewAdapter.getItemCount() - 1), offset);
        }
    }

    private void updateExplorerContentWidthForDisplayMode() {
        if (!is720dp || swipeRefreshLayout == null) {
            return;
        }

        boolean useFullWidthGrid = fileViewMode == FileExplorerRecyclerViewAdapter.VIEW_MODE_GRID
                && getResources().getConfiguration().orientation
                == Configuration.ORIENTATION_LANDSCAPE;
        int targetWidth = useFullWidthGrid
                ? ViewGroup.LayoutParams.MATCH_PARENT
                : getResources().getDimensionPixelSize(R.dimen.file_explorer_tablet_list_width);

        ViewGroup.LayoutParams rawLayoutParams = swipeRefreshLayout.getLayoutParams();
        if (rawLayoutParams.width == targetWidth) {
            return;
        }
        rawLayoutParams.width = targetWidth;
        if (rawLayoutParams instanceof CoordinatorLayout.LayoutParams) {
            ((CoordinatorLayout.LayoutParams) rawLayoutParams).gravity = Gravity.CENTER_HORIZONTAL;
        }
        swipeRefreshLayout.setLayoutParams(rawLayoutParams);
    }

    private void updateViewModeMenuChecks(Menu menu) {
        if (menu == null) {
            return;
        }
        int checkedId;
        if (fileViewMode == FileExplorerRecyclerViewAdapter.VIEW_MODE_LIST) {
            checkedId = R.id.action_view_list;
        } else {
            switch (thumbnailDisplaySize) {
                case FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_SMALL:
                    checkedId = R.id.action_view_icons_small;
                    break;
                case FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_LARGE:
                    checkedId = R.id.action_view_icons_large;
                    break;
                case FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_EXTRA_LARGE:
                    checkedId = R.id.action_view_icons_extra_large;
                    break;
                case FileExplorerRecyclerViewAdapter.THUMBNAIL_SIZE_MEDIUM:
                default:
                    checkedId = R.id.action_view_icons_medium;
                    break;
            }
        }
        ...
        MenuItem viewModeItem = menu.findItem(R.id.action_view_mode);
        if (viewModeItem != null) {
            viewModeItem.setIcon(fileViewMode == FileExplorerRecyclerViewAdapter.VIEW_MODE_LIST
                    ? R.drawable.ic_round_list_24 : R.drawable.ic_view_grid);
        }
    }
    ...
    // serve callback
    @Override
    public void onServeOptionsSelected(int protocol, boolean allowRemoteAccess, boolean readOnly, String user, String password) {
        ...
        intent.putExtra(StreamingService.READ_ONLY, readOnly);
    ...
    private void cancelMoveClicked() {
        ...
        if (moveStartPath != null && !moveStartPath.equals(directoryObject.getCurrentPath())) {
            ...
            if (directoryObject.isPathInCache(moveStartPath)) {
                restoreCurrentDirectoryFromCache(moveStartPath);
                recyclerViewAdapter.newData(directoryObject.getDirectoryContent());
            } else {
                setCurrentDirectoryPath(moveStartPath);
    ...
    public void onDestroyView() {
        ...
        if (fetchDirectoryTask != null) {
            fetchDirectoryTask.cancel(true);
            fetchDirectoryTask = null;
        }

        // These controls live in MainActivity rather than in this Fragment's
        // layout. A Fragment kept on the back stack is not detached, so waiting
        // for onDetach() would leave the file breadcrumb/search UI visible over
        // the log page.
        hideFileExplorerChrome();
    ...
    private void hideFileExplorerChrome() {
        if (breadcrumbView != null) {
            breadcrumbView.clearCrumbs();
    ...
    public boolean onBackButtonPressed() {
        ...
        if (!directoryObject.isContentValid(path)) {
            swipeRefreshLayout.setRefreshing(true);
            restoreCurrentDirectoryFromCache(path);
            sortDirectory();
            recyclerViewAdapter.newData(directoryObject.getDirectoryContent());
    ...
    public void onDirectoryClicked(FileItem fileItem, int position) {
        ...
        if (!directoryObject.isContentValid(fileItem.getPath())) {
            swipeRefreshLayout.setRefreshing(true);
            restoreCurrentDirectoryFromCache(fileItem.getPath());
            sortDirectory();
            recyclerViewAdapter.newData(directoryObject.getDirectoryContent());
            fetchDirectoryTask = new FetchDirectoryContent(true).execute();
            ...
            setCurrentDirectoryPath(fileItem.getPath());
            recyclerViewAdapter.clear();
            fetchDirectoryTask = new FetchDirectoryContent().execute();
        }
    }

    private void startRecursiveThumbnailGeneration(ThumbnailRepository.CacheType cacheType,
                                                   String rootPath) {
        if (context == null || remote == null || rootPath == null || rootPath.isEmpty()) {
            return;
        }

        ThumbnailRepository repository = ThumbnailRepository.getInstance(context);
        boolean enabled = cacheType == ThumbnailRepository.CacheType.IMAGE
                ? repository.areImageThumbnailsEnabled()
                : repository.areVideoThumbnailsEnabled();
        if (!enabled) {
            int disabledMessage = cacheType == ThumbnailRepository.CacheType.IMAGE
                    ? R.string.thumbnail_recursive_image_disabled
                    : R.string.thumbnail_recursive_video_disabled;
            Toasty.warning(context, getString(disabledMessage), Toast.LENGTH_LONG, true).show();
            return;
        }
        ...
        int submittedMessage = cacheType == ThumbnailRepository.CacheType.IMAGE
                ...
                : R.string.thumbnail_recursive_video_submitted;
        Toasty.info(context, getString(submittedMessage), Toast.LENGTH_SHORT, true).show();
    }
    ...
    private void showFileMenu(View view, final FileItem fileItem) {
        ...
        popupMenu.setOnMenuItemClickListener(item -> {
            switch (item.getItemId()) {
                ...
                case R.id.action_open_as:
                    ...
                    break;
                case R.id.action_regenerate_thumbnail:
                    Toasty.info(
                            context,
                            getString(R.string.regenerating_thumbnail),
                            Toast.LENGTH_SHORT,
                            true).show();
                    recyclerViewAdapter.regenerateVideoThumbnail(fileItem, success -> {
                        ...
                        if (success) {
                            Toasty.success(
                                    context,
                                    getString(R.string.thumbnail_regenerated),
                                    Toast.LENGTH_SHORT,
                                    true).show();
                        } else {
                            Toasty.error(
                                    context,
                                    getString(R.string.thumbnail_regeneration_failed),
                                    Toast.LENGTH_SHORT,
                                    true).show();
                        }
                    });
                    break;
                case R.id.action_recursive_image_thumbnails:
                    startRecursiveThumbnailGeneration(
                            ThumbnailRepository.CacheType.IMAGE, fileItem.getPath());
                    break;
                case R.id.action_recursive_video_thumbnails:
                    startRecursiveThumbnailGeneration(
                            ThumbnailRepository.CacheType.VIDEO, fileItem.getPath());
                    break;
                case R.id.action_serve:
        ...
        popupMenu.getMenu().findItem(R.id.action_regenerate_thumbnail).setVisible(
                recyclerViewAdapter.canRegenerateVideoThumbnail(fileItem));
        boolean isDirectory = fileItem.isDir();
        popupMenu.getMenu().findItem(R.id.action_recursive_image_thumbnails)
                .setVisible(isDirectory);
        popupMenu.getMenu().findItem(R.id.action_recursive_video_thumbnails)
                .setVisible(isDirectory);
        if (isDirectory) {
    ...
    private static String createStreamPathToken() {
        byte[] tokenBytes = new byte[STREAM_PATH_TOKEN_BYTES];
    ...
    private class StreamTask extends AsyncTask<FileItem, Void, Boolean> {
        ...
        protected Boolean doInBackground(FileItem... fileItems) {
            ...
            serveIntent.putExtra(StreamingService.VIDEO_STREAM_READ_CONFIG, isVideoStream(fileItem));
            // Use a 256-bit capability path. rclone --baseurl rejects requests that do not
            // present this token, so knowing only the localhost port is not enough to read data.
            String streamPathToken = createStreamPathToken();
            serveIntent.putExtra(StreamingService.SERVE_BASE_URL, streamPathToken);
            // GH-87: Release old stream
            context.stopService(new Intent(context, StreamingService.class));
            ...
            while (waitTime > 0) {
                ...
                try {
                    FLog.v(TAG, "doInBackground: checking protected local stream on port %d", port);
        ...
        private boolean isVideoStream(FileItem item) {
            if (openAs == OPEN_AS_VIDEO) {
                return true;
            }
            if (openAs == OPEN_AS_AUDIO) {
                return false;
            }
